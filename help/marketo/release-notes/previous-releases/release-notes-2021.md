...
* 工作区支持：Marketo Engage的新一代用户体验将Adobe Experience Cloud的外观与工作效率创新相结合，帮助营销从业人员更快、更智能地工作。 在最新版本中，我们将添加对工作区和分区的完全支持，包括跨工作区共享文件夹的功能。 右侧画布将提供一个切换开关，允许您在每个功能的新旧体验之间无缝过渡，而不会丢失上下文。 [从有关Marketing Nation的新一代体验常见问题解答中了解更多信息](https://nation.marketo.com/t5/The-modern-ux/modern-ux-FAQ/ba-p/307124)。
...
* **电子邮件连接限制(BETA)**：通过[!DNL Sales Connect]的电子邮件连接限制，提高电子邮件可投放性并扩展1:1销售通信。 我们的新限制技术可自动管理电子邮件发送时间，为[!DNL Exchange]和Gmail用户创造无缝体验。 减少或消除使用第三方批量电子邮件发送应用程序。
...
* **计划成员资格**：营销人员可以根据不同的条件（如计划成员状态）查询选定计划的所有计划成员记录。 与外部应用程序、商业智能工具或Adobe Experience Cloud共享此信息，以改进分段并创建更有针对性的参与。
...
* **潜在客户API**：识别Adobe Marketo Engage中具有Adobe ECID (Experience Cloud ID)关联的潜在客户。  Adobe Marketo Engage客户可以通过选定的营销活动创建潜在客户列表，并使用ECID (Experience Cloud ID)在Adobe Analytics中为该特定列表创建报表。 Adobe Marketo Engage与Adobe Experience Cloud之间的集成为分段、定位和报表带来了无限商机。
...
* **由Adobe Identity System提供支持的简化用户管理和单点登录**：从2021年7月开始，将使用Adobe Marketo Engage用户凭据登记新的Adobe客户。 在2022年年中之前不会将当前客户迁移到集成的Identity System，在进一步通知之前，无需客户采取任何行动。 通过单点登录，IT/安全管理员可以管理多个Adobe Marketo Engage产品实例以及其他Experience Cloud解决方案，并通过通用控制台配置SSO(Shared Services Organization)。 管理员可以通过公共Admin Console方便地管理用户组和用户权限。
...
## [!DNL Bizible]与Adobe Experience Cloud的集成 {#bizible-integration-with-adobe-experience-cloud}
...
* **[!DNL Bizible]与Adobe Privacy Service**&#x200B;的集成（2021年9月推出）： [!DNL Bizible]与Adobe Privacy Service的集成可集中管理跨Adobe Experience Cloud应用程序对关键数据隐私法规（如GDPR）的合规性。 现在，您可以利用此服务并集中管理所有隐私请求，以便跨应用程序反映传入[!DNL Bizible]和其他Adobe产品的更改请求。
...
* **[!DNL Bizible]域所有权和自我管理**： [!DNL Bizible]用户可以利用Adobe Admin Console管理他们希望[!DNL Bizible]跟踪的域。 这将自助服务引入以前的手动流程，并在如何跨Adobe Experience Cloud应用程序管理域所有权和跟踪方面提供一致的体验。
...
2021年10月版本中包含以下功能。 请检查您的 Adobe Marketo Engage 版本以确认功能可用性。
