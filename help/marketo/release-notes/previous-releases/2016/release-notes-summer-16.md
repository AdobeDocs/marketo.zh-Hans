...
2016年夏季版本中包含以下功能。 查看Marketo版本以了解功能可用性。 请单击标题链接以查看每个功能的详细文章。
...
>[!NOTE]
>
>ABM可作为您的Marketo订阅的附加组件提供，因此请与您的销售代表联系以实施它。

## [审核记录](/help/marketo/product-docs/administration/audit-trail/audit-trail-overview.md) {#audit-trail}

审核记录提供了Marketo订阅中所做更改的全面历史记录。 它将创建用户和管理员之间的问责制，帮助识别意外行为的原因，并提供了解谁在做什么以及何时做什么的安全性。 此信息将在任何时间点提供，并可用于回答以下问题：
...
直接在Marketo中轻松创建短信消息。 使用丰富的Marketo数据对消息进行个性化和定位，并使用SMS消息仪表板轻松监控其性能。
...
以前，在Email 2.0模板中指定的所有变量在范围中均为“全局”变量。 在模块中使用变量时，如果您计划使用模块的多个实例，并非总是希望这样做。 在此版本中，现在可以将变量指定为“模块级别”，这样可让您指示用户应能够为其使用的每个模块设置唯一值。

![](assets/module-level-variables.png)

**语法更新**

* 现在，您可以对Email 2.0模板中指定的模块使用“mktoAddByDefault”，以指示默认情况下应在新电子邮件中显示哪些模块。 如果您构建包含大量模块的电子邮件模板，这会更加方便。
* 在图像元素上，您现在可以指定基础 `<img>` HTML元素的“height”和“width”属性应该锁定或者可供最终用户编辑。 mktoLockImgSize=&quot;true&quot;将导致高度/宽度被锁定（即使图像已更改）。 同样，mktoLockImgStyle=&quot;true&quot;将导致锁定&quot;style&quot;属性。
...
令牌现在可用于插入图像体验的“外部URL”区域！ 如果您已指定图像 `{{my.tokens}}`中，您现在可以在电子邮件编辑器2.0中引用这些令牌。请注意，在电子邮件编辑器2.0画布中，图像仍显示为已损坏。 但是，在发送电子邮件之前，您会在预览和发送示例中看到这些缩图。

## 多个品牌推广域 {#multiple-branding-domains}

电子邮件跟踪链接只能使用单个品牌域进行标记的日子已经一去不复返了。 您现在可以添加多个品牌域来激发消费者的信心，创建更加简化的外观以专注于品牌，提高电子邮件的可投放性，并根据电子邮件选择用于每个电子邮件的跟踪链接的品牌域。
...
我们为项目创建了新的令牌类型。 您现在可以在资源和智能营销活动流程步骤中渲染项目名称、描述和ID。
...
要求您的销售团队中的每个人安装我们的Sales Insight Plugin for Outlook可能会非常繁琐。 我们引入了一种使用企业密钥远程安装Outlook插件的新方法。 向IT团队发送您在“管理员”的Marketo Sales Insight部分找到的唯一密钥，然后让他们完成其余任务。
...
营销人员可以使用隐私设置决定是否使用Munchkin和Web个性化功能跟踪访客。 跟踪级别通过使用浏览器的Do Not Track设置、选择退出Cookie或非特定IP进行控制。 这些方法可能会影响Marketo在特定领域的价值和功能，但是，如果营销人员不更改任何内容，则Marketo功能将保持不变。

此功能将在六周后逐步向客户发布。 如果您需要立即获得支持，请联系Marketo支持。
