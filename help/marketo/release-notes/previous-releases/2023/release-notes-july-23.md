...
* **通知 — Web服务错误**： Web服务错误通知现在包含订阅名称，这使得管理多个Marketo Engage实例的用户更容易操作。 我们还向包含错误代码和计数的表添加了“用户”列，以便您能够轻松识别在调用Marketo REST API时哪个API用户负责生成特定错误。
...
* **Dynamic Chat的Source聊天**：“聊天”现在作为新人员记录和活动的源提供。 此源可用于帮助用户过滤智能列表/营销活动中的活动。 还可以阻止源更新字段。
...
* **取消订阅Sales Insight操作的附加设置**：您现在可以通过新的配置选项“取消订阅附加”更好地控制取消订阅消息。 新功能使管理员能够控制何时何地以及在您的销售电子邮件中显示取消订阅消息。
...
* **销售分析操作的动态字段提示**：现在，您可以利用可添加到销售分析操作中电子邮件模板的新字段提示，提示销售团队进一步个性化电子邮件的特定部分。
...
* **Dynamic Chat对Sales Insight的支持**：已进行更新，以支持所有Dynamic Chat用户在此版本中引入的新Dynamic Chat属性。
...
* **Dynamic Chat支持Sales Insight Actions**：我们现在支持潜在客户信息源中的Dynamic Chat活动，以及在将Live Chat路由到代理时的通知。
...
* **Dynamic Chat触发器令牌**：为要在有趣时刻使用的Dynamic Chat活动引入新的触发器令牌。
...
* **升级到Google Analytics4**：Marketo以前支持Universal Analytics，后者将在2023年7月1日之前停止记录数据。 RTP用户将需要遵循其Google Analytics功能板中的迁移过程，以便完全支持RTP中的Google Analytics4集成。 今后，用户在其登陆页面中将有一个唯一的字母数字标记（位于html的`<head>`部分中），允许Marketo向Google Analytics发送数据。
