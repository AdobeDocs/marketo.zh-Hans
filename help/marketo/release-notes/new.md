---
description: 新发行说明 — Marketo文档 — 产品文档
title: 新发行说明
hide: true
hidefromtoc: true
...
* **升级到Google Analytics4**： RTP用户将具有唯一的字母数字标记(位于 `<head>` 部分)，允许Marketo向Google Analytics发送数据。 Marketo以前使用过Universal Analytics，后者将在2023年7月1日之前停止记录数据。 RTP用户将需要遵循其Google Analytics功能板中的迁移过程来完成迁移。

* **升级到jQuery 3.7**：为了提高安全性和稳定性，Marketo正在升级到jQuery 3.7.0。自行加载jQuery的RTP用户如果使用的版本低于3.0.0，则可能会受到负面影响。这些用户可以联系Marketo支持，以获得实施特定版本rtp.js以满足其需求的帮助。 如果您使用的是版本3.0.0或更高版本，则不会受到影响。
