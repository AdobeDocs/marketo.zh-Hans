---
unique-page-id: 4718668
description: 了解如何使用收入模型sla阶段在Marketo Engage中使用收入模型sla阶段。 使用本指南完成您的下一步。
title: 使用收入模型 SLA 阶段
exl-id: 48ca4be9-0e77-4065-8769-ce046d0c50ea
feature: Reporting, Revenue Cycle Analytics
source-git-commit: 50befbf7339cd7a8b25b0942515497f6acc8f9ab
workflow-type: tm+mt
source-wordcount: '239'
ht-degree: 4%

---

# 使用收入模型 SLA 阶段 {#using-revenue-model-sla-stages}

SLA表示“service level agreement”。 当存在定义的最大时间时，在向前或退出流程之前需要评估或处理潜在客户，则使用这些阶段。

>[!TIP]
>
>最好在图形或演示程序中创建一个实践模型，并与同事确认。

## 添加SLA Stage {#add-an-sla-stage}

1. 要添加新的收入周期模型SLA阶段，请单击&#x200B;**[!UICONTROL Analytics]**&#x200B;主屏幕中的[!UICONTROL My Marketo]按钮。

   ![](assets/image2015-4-27-11-3a54-3a41.png)

1. 在[!UICONTROL Analytics]部分中，选择现有模型，或[创建一个新模型](/help/marketo/product-docs/reporting/revenue-cycle-analytics/revenue-cycle-models/create-a-new-revenue-model.md)。

   ![](assets/image2015-4-27-15-3a6-3a30.png)

1. 单击 **[!UICONTROL Edit Draft]**。

   ![](assets/image2015-4-27-12-3a10-3a49.png)

1. 要添加新的SLA暂存，请单击&#x200B;**[!UICONTROL SLA]**&#x200B;按钮，然后将其拖放到画布中的任意位置。

   ![](assets/image2015-4-27-15-3a32-3a10.png)

1. 您可以在添加阶段后随意编辑&#x200B;**[!UICONTROL Name]**、添加&#x200B;**[!UICONTROL Description]**&#x200B;并调整&#x200B;**[!UICONTROL Type]**。 此时您还可以选择&#x200B;**[[!UICONTROL Start Tracking by Account]](/help/marketo/product-docs/reporting/revenue-cycle-analytics/revenue-cycle-models/start-tracking-by-account-in-the-revenue-modeler.md)**。

   ![](assets/image2015-4-27-17-3a0-3a39.png)

## 编辑SLA Stage {#edit-an-sla-stage}

选择SLA阶段图标时，您可以编辑&#x200B;**[!UICONTROL Name]**、添加&#x200B;**[!UICONTROL Description]**&#x200B;并调整&#x200B;**[!UICONTROL Type]**。 您还可以选择[[!UICONTROL Start Tracking by Account]](/help/marketo/product-docs/reporting/revenue-cycle-analytics/revenue-cycle-models/start-tracking-by-account-in-the-revenue-modeler.md)。

1. 单击SLA Stage图标。

   ![](assets/image2015-4-27-15-3a45-3a25.png)

1. 单击&#x200B;**[!UICONTROL Name]**&#x200B;和&#x200B;**[!UICONTROL Description]**&#x200B;字段以编辑其内容。

   ![](assets/image2015-4-27-15-3a48-3a37.png)

1. 选择要编辑的&#x200B;**[!UICONTROL Type]**&#x200B;下拉菜单。

   ![](assets/image2015-4-27-15-3a51-3a27.png)

## 删除SLA Stage {#delete-an-sla-stage}

1. 您可以通过右键单击或按住Control键单击SLA Stage图标来删除SLA Stage。

   ![](assets/image2015-4-27-16-3a2-3a47.png)

1. 也可以通过单击某个阶段将其删除，然后在&#x200B;**[!UICONTROL Stage Actions]**&#x200B;下拉菜单中选择&#x200B;**[!UICONTROL Delete]**。

   ![](assets/image2015-4-27-17-3a20-3a41.png)
