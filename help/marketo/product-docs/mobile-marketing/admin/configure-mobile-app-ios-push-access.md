---
unique-page-id: 7512434
description: 配置移动应用程序iOS推送访问 — Marketo文档 — 产品文档
title: 配置移动应用程序iOS推送访问
exl-id: d8c54232-3df2-4e25-ab25-3e72aaf49252
feature: Mobile Marketing
source-git-commit: 431bd258f9a68bbb9df7acf043085578d3d91b1f
workflow-type: tm+mt
source-wordcount: '141'
ht-degree: 0%

---

# 配置移动应用程序iOS推送访问 {#configure-mobile-app-ios-push-access}

1. 单击 **管理员**.

   ![](assets/image2015-4-22-16-3a12-3a32.png)

1. 选择 **移动应用程序**.

   ![](assets/image2015-4-22-16-3a14-3a29.png)

1. 选择所需的移动设备应用程序。

   ![](assets/image2015-4-22-16-3a33-3a19.png)

1. 在推送访问类型下，选择iOS ，然后单击 **配置**.

   ![](assets/image2016-6-10-11-3a37-3a9.png)

   >[!NOTE]
   >
   >您需要一个 **证书** 和 **密码** 来自您的移动应用程序开发人员。 开发人员通过登录Apple开发人员成员中心、为您的应用程序设置和下载推送通知证书并导出内容来接收这些内容。 开发人员在导出时设置密码。 **重要**：证书应该适合您使用的环境类型 — 沙盒或生产。 请与您的Marketo管理员或移动应用程序开发人员确认。

1. 选择您的证书，输入密码，然后单击 **保存**.

   ![](assets/image2015-4-22-17-3a19-3a18.png)

做得好！ 确保还使用Android配置应用程序。

>[!MORELIKETHIS]
>
>[配置移动应用程序Android推送访问](/help/marketo/product-docs/mobile-marketing/admin/configure-mobile-app-android-push-access.md)
