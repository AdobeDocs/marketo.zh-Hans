...
如果您的Adobe Marketo Engage帐户通过自定义或非本机集成连接到Salesforce，请使用本文配置Sales Insight。
...
>* 具有的Salesforce帐户 [设置MSI包](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/installation/install-marketo-sales-insight-package-in-salesforce-appexchange.md){target=&quot;_blank&quot;}。
...
1. 将Salesforce Sales用户同步到Marketo。
   ...
   Salesforce Sales User是在Salesforce中拥有Lead/Contacts的外部用户。 需要为Salesforce销售用户更新Marketo销售人员。 的 *externalSalesPersonId* 字段用于添加销售人员。
...
1. 将Salesforce帐户同步到Marketo。
   ...
   需要更新Marketo公司的Salesforce帐户。 的 _externalCompanyId_ 和 _externalSalesPersonId_ 字段用于更新公司。
...
1. 将Salesforce潜在客户/联系人同步到Marketo。
   ...
   您需要为Salesforce潜在客户/联系人添加Marketo潜在客户。 的 _externalPersonId_, _externalSalesPersonId_&#x200B;和 _externalCompanyId_ 字段用于Lead的插入。
...
1. 将Salesforce机会同步到Marketo。
   ...
   您需要为Salesforce Opportunity插入Marketo Opportunity。 的 _externalOpportunityId_, _externalCompanyId_&#x200B;和 _externalSalesPersonId_ 字段被强制用于Opportunity的更新。
...
1. 将上一个有趣的时刻/MSI评分字段同步到SFDC。
   ...
   Marketo Lead的“最后一个有趣的时刻/评分”字段将需要使用REST API Lead端点定期同步到Salesforce。 使用查询Marketo Lead的此端点 _externalPersonId_ 作为filterType，并将Salesforce潜在客户GUID作为filterValue传递。
