...
**发生的情况：**&#x200B;访问或刷新令牌已过期。 令牌将根据 [!DNL Salesforce]](https://salesforce.stackexchange.com/questions/10759/invalid-grant-expired-access-refresh-token-error-when-authenticating-access-via)中的[会话设置过期。
**疑难解答步骤：**&#x200B;您需要重新进行身份验证。 断开[!DNL Salesforce]连接并重新连接。

**错误：** FAILED_WRITE
**类别：**间断
**消息：**已到达文件结尾
**发生什么情况：** [!DNL Salesforce]出现性能问题，可能是由于客户端触发程序处于次优状态。
...
**类别：**访问/验证
**消息：**值不存在或与筛选条件不匹配
**发生的情况：**&#x200B;更新时强制执行[!DNL Salesforce]中的现有错误数据。
**疑难解答步骤：**&#x200B;请参阅上文。
...
**发生什么情况：**&#x200B;我们正在尝试访问您的[!DNL Salesforce]，但您设置了IP限制阻止我们执行此操作。
...
**类别：**访问/验证
**消息：**由于组织中存在安全策略，不允许执行您请求的操作。 请联系您的管理员。
...
**类别：**其他
**消息：**发生未知异常
**发生的情况：[!DNL Salesforce]中出现**未处理的异常。
