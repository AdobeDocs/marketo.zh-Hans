...
**类别：**&#x200B;访问/验证\
**消息：**&#x200B;实体被锁定以进行编辑\
**发生什么情况：**&#x200B;记录处于审批流程中，在获得审批负责人的批准或拒绝之前，该记录不会受到任何其他编辑。\
**疑难解答步骤：**&#x200B;请参阅上文。

**错误：** EXPIRED_ACCESS
**类别：**身份验证
**消息：** invalid_grant：访问/刷新令牌已过期
**发生的情况：**&#x200B;访问或刷新令牌已过期。 令牌将根据Salesforce](https://salesforce.stackexchange.com/questions/10759/invalid-grant-expired-access-refresh-token-error-when-authenticating-access-via)中的[会话设置过期。
**疑难解答步骤：**&#x200B;您需要重新进行身份验证。 断开Salesforce连接，然后重新连接。

**错误：** FAILED_WRITE\
**类别：**&#x200B;间断\
**消息：**&#x200B;已到达文件结尾\
**发生什么情况：** Salesforce出现性能问题，可能是由于客户端的触发器性能缺佳。\
...
**疑难解答步骤：**&#x200B;在Salesforce中授予用户访问权限或禁用该用户对该对象的最近活动日志记录。
...
**发生的情况：**&#x200B;用户没有执行最近活动日志记录事务所需的销售分析操作自定义字段的写入权限。 团队可能已经安装了软件包，但尚未为用户启用正确的字段。\
**疑难解答步骤：** Salesforce管理员需要授予对自定义字段的访问权限或关闭最新活动日志记录。

**错误：** INVALID_GRANT\
**类别：**&#x200B;身份验证\
**消息：** invalid_grant： ip受限\
**发生什么情况：**&#x200B;我们正在尝试访问您的Salesforce，但您设置了IP限制阻止我们执行此操作。\
**故障排除步骤：**&#x200B;您的Salesforce管理员需要允许列表我们的IP。 用户应联系支持人员以获取IP地址。
...
**类别：**&#x200B;间断\
**消息：**&#x200B;服务器太忙\
**发生什么情况：** Salesforce出现性能问题，可能是由于客户触发的次优所致\
...
**类别：**&#x200B;其他\
**消息：**&#x200B;发生未知异常\
**发生什么情况：** Salesforce中未经处理的异常。\
**疑难解答步骤：**&#x200B;使用Salesforce归档案例并复制错误消息中的数值。 这是Salesforce代码未正确处理错误。
