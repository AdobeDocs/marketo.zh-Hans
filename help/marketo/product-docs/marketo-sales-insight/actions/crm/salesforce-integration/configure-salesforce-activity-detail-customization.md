...
>[!PREREQUISITES]
>
>* Salesforce和Sales Insight操作 [必须连接](/help/marketo/product-docs/marketo-sales-insight/actions/crm/salesforce-integration/connect-your-marketo-sales-account-to-salesforce.md)
>* 通过API记录电子邮件活动 [必须启用](/help/marketo/product-docs/marketo-sales-insight/actions/crm/salesforce-integration/salesforce-sync-settings.md)


活动详细信息自定义允许管理员配置在Salesforce活动/提醒任务同步到Salesforce时，将记录到Salesforce任务 — 主题字段的信息。
...
* 管理员可以使用唯一标识符（如“Mkto_sales”）来标记主题字段，以便能够轻松识别Sales Insight Actions中的活动，并将其与其他电子邮件活动、呼叫活动和任务区分开来。
...
1. 在活动详细信息自定义编辑器中，添加您需要的任何自由文本。 您添加的文本是非动态的，对于同步到Salesforce的所有任务的主题字段，其文本将保持不变。
   ...
   >[!TIP]
   >
   >虽然不需要，但将添加的文本换行为在直括括号中，这样当数据填充到Salesforce中的主题字段时，有些人就可以更轻松地在数据之间进行识别。 示例： `[Sales Insight Actions] - {{Activity_type}}`
...
>[!MORELIKETHIS]
>
>* [同步设置](/help/marketo/product-docs/marketo-sales-insight/actions/crm/salesforce-integration/salesforce-sync-settings.md)
