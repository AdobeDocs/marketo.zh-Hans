---
description: 从Salesforce Classic卸载Marketo Sales Connect - Marketo文档 — 产品文档
...
下面介绍了在开始使用Sales Insight Actions后，如何从Salesforce帐户卸载Marketo Sales Connect软件包。
...
1. 在左侧导航中的Build下，展开（但不单击）Customize，然后展开Leads。 然后，选择 **页面布局**.
...
1. 在左侧导航中的Build下，展开（但不单击）Customize，然后展开Leads。 然后，选择 **搜索布局**.
...
1. 选择 **添加到MSC Campaign (Classic)**， **使用MSC (Classic)发送电子邮件**、和 **推送到MSC（经典）**，然后单击删除箭头。 然后单击 **保存**.
...
1. 在左侧导航中的“构建”下，依次展开（但不单击）自定义、联系人。 然后，选择 **页面布局**.
...
1. 在左侧导航中的Build下，展开（但不单击）Customize，然后展开Opportunities。 然后，选择 **页面布局**.
...
1. 在左侧导航中的生成下，展开（但不单击）自定义，然后展开帐户。 然后，选择 **页面布局**.
