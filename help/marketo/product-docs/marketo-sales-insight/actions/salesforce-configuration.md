---
description: Salesforce配置 — Marketo文档 — 产品文档
title: Salesforce配置
hide: true
hidefromtoc: true
...
## 卸载Salesforce自定义包(Classic) {#uninstall-salesforce-customization-package-classic}

开始使用MSI-Actions包后，请从Salesforce帐户中卸载Marketo Sales Connect包。

**从“页面布局”中删除Sales Connect字段**
...
**从活动历史记录部分删除Sales Connect字段**

1. 滚动到页面底部，查看与活动历史记录相关的列表部分，然后单击扳手图标。

   ![](assets/salesforce-configuration-10.png)

1. 接下来，从“选定字段”区域选择“Sales Connect Fields”（销售连接字段），然后单击“Remove（删除）”箭头。 单击 **确定** 完成时。
...
**从潜在客户列表视图中删除Sales Connect批量操作按钮**
