...
1. 输入远程站点名称（可以是类似“MarketoRestAPI”的内容）。 输入远程站点URL，即Marketo中Rest API配置面板中的API URL。 单击 **保存**. 您现在已为Rest API创建了远程站点设置。

## 设置Marketo Sales Insight {#set-up-marketo-sales-insight}

1. 登录到Marketo实例，然后单击 **管理员**.
...
1. 输入 **API密钥** 单击 **保存**.
   ...
   >[!TIP]
   >
   >保持此窗口打开。 您稍后将在Salesforce中需要此信息。

1. 返回至Salesforce，单击 **设置**.

   ![](assets/image2015-5-22-14-3a40-3a39-1.png)

1. 搜索“远程站点”并单击 **远程站点设置** 在 **安全控制**.
...
1. 输入 **远程站点名称** 和 **远程站点URL**，然后单击 **保存**.
   ...
   >[!NOTE]
   >
   >您选择 **远程站点名称** （此处使用MarketoAPI）。 的 **远程站点URL** 可在步骤4的“编辑API配置”对话框的“Marketo主机”字段中找到。
...
1. 将高度编辑为 **450** 像素和单击 **确定**.
   ...
   >[!NOTE]
   >
   >检查 **显示滚动条** 如果您需要访问滚动浏览活动，请执行以下操作：
