...
description: 了解智能列表中推断的筛选器。 了解Marketo如何推断来自链接资源的筛选器。
title: 推断过滤器
...
当有人访问您的网站时，[Munchkin](/help/marketo/product-docs/administration/additional-integrations/add-munchkin-tracking-code-to-your-website.md){target="_blank"}会对其进行Cookie并将其放入系统中。 该系统在一个专门的数据库中查找他们的IP，并推断各种信息。

>[!NOTE]
>
>为确保推断的字段值保持最新，将定期更新用于IP地址查找的数据库。 数据库更新可能会引入新的推断字段值，您可能需要将这些值添加到智能列表筛选器定义中。
>
