...
通过将智能列表规则逻辑应用于智能列表中的多个筛选器，您可以找到所需的确切人员。 下面是具体操作方法。
...
>[!NOTE]
>
>仅当智能列表中有三个或更多过滤器时，高级过滤器逻辑才可用。

## 将逻辑添加到智能列表 {#add-logic-to-a-smart-list}

默认情况下，您的智能列表将查找匹配的人员 **全部** 过滤器(过滤器1 _和_ 2 _和_ 3)。 您可以更改规则逻辑以查找匹配的人员 **任意** 已定义的过滤器（过滤器1） _或_ 2 _或_ 3)，或使用高级过滤器(过滤器1 _和_ 2 _或_ 3)。

在此示例中，假设您希望查找加利福尼亚的人员 _和_ 得分至少为50分 _或_ 状态为“符合销售条件”。
...
1. 选择 **使用高级过滤器** 从下拉菜单中。
   ...
   >[!NOTE]
   >
   >使用 **高级** 过滤器可减少使用智能列表过滤器成员创建智能列表的需求。 这有助于优化性能。
...
将“and”和“or”逻辑混合在一起时，需要用括号来表示意图。
