...
>[!IMPORTANT]
>
>目前，您只能构建营销活动，不能发送（启动）它们。 Launch功能即将推出。
...
>[!NOTE]
>
>如果您已经拥有Campaign Brief，请将其与提示一起上载，作为为您创建的计划的附加上下文。

![输入您自己的提示](assets/create-an-email-campaign-2.png)

提示就绪后，单击&#x200B;**生成营销活动**。 然后， CX Enterprise Co-worker将：
...
1. 在左侧的&#x200B;_促销活动转化_&#x200B;窗格中，单击&#x200B;**上传CSV**。
   ...
   >[!NOTE]
   >
   >* 电子邮件地址是必填字段，建议输入名字以及可用于个性化的其他字段。
   >
   >* CX Enterprise Co-worker可以使用的个性化字段：名字、上次订购日期、产品类别。
...
* 使用对话式界面，直接与CX Enterprise Co-worker交谈以进行更改。 一些示例包括：
...
它正在为您生成整个营销活动，包括策略、所需受众、工作流等（收听记录1:15ish标记）

**如果CX Enterprise Co-worker输出不太正确，我该怎么做？**

使用右上角的反馈按钮通知我们，以便我们改进平台。
