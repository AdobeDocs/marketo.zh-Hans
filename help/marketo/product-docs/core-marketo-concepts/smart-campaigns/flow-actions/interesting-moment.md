---
unique-page-id: 1146956
description: 有趣的时刻 — Marketo Docs — 产品文档
title: 有趣的时刻
exl-id: fa1a45b8-1e62-497e-9441-bf6859addb42
translation-type: tm+mt
source-git-commit: 4fc3cf6e6458f07df7cced9399831b8c6b50e0ad
workflow-type: tm+mt
source-wordcount: '98'
ht-degree: 0%

---

# 有趣的时刻{#interesting-moment}

## 概述{#overview}

如果您有Marketo Sales Insight，则可以使用&#x200B;**有趣的时刻**&#x200B;流步骤让您的销售团队了解您的员工正在做的事情。

![](assets/image2016-1-27-11-3a0-3a21.png)

## 用法{#usage}

1. 选择要使用的有趣时刻&#x200B;**类型**。

   ![](assets/image2014-9-22-10-3a49-3a51.png)

1. 创建&#x200B;**description**，以说明您的销售团队所感兴趣的时刻。

   ![](assets/image2014-9-22-10-3a49-3a55.png)

   >[!TIP]
   >
   >**少是多**。与您的销售团队合作，确保有趣的时刻真正有趣。

您还可以在有趣的时刻](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/trigger-tokens-for-interesting-moments.md)使用[令牌来做出真正有用的动态描述。

>[!MORELIKETHIS]
>
>* [使用有趣的瞬间](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/using-interesting-moments.md)
>* [有趣时刻的令牌](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/trigger-tokens-for-interesting-moments.md)

