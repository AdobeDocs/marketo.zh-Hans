---
unique-page-id: 2951703
description: 请求营销活动 — Marketo文档 — 产品文档
title: 请求营销活动
exl-id: 5deecb56-c25f-4e3b-bd85-f9cac1c7149e
feature: Smart Campaigns
source-git-commit: 12f2399859c784095cc2c1df772c66c649106ba3
workflow-type: tm+mt
source-wordcount: '116'
ht-degree: 0%

---

# 请求营销活动 {#request-campaign}

您可以使用“[!UICONTROL 请求营销活动]”单流程步骤将人员放入特定营销活动。

>[!NOTE]
>
>您要将人员放置到其中的营销活动必须处于活动状态，并且具有[已请求营销活动](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/using-smart-campaigns/setting-up-a-trigger-smart-campaign-for-sales-using-campaign-is-requested.md){target="_blank"}触发器。

1. 在&#x200B;**[!UICONTROL 数据库]**&#x200B;中，查找并选择所需的人员。

   ![](assets/request-campaign-1.png)

1. 单击&#x200B;**[!UICONTROL 人员操作]**，将鼠标悬停在&#x200B;**[!UICONTROL 特殊优惠]**&#x200B;上，然后选择&#x200B;**[!UICONTROL 请求营销活动]**。

   ![](assets/request-campaign-2.png)

1. 选择您希望此人参与的营销活动，然后单击&#x200B;**[!UICONTROL 立即运行]**。

   ![](assets/request-campaign-3.png)

1. 在屏幕的右上角，您将会看到操作何时完成。 单击&#x200B;**[!UICONTROL 查看结果]**。

   ![](assets/request-campaign-4.png)

   您的人员现在位于请求的营销活动中。

   ![](assets/request-campaign-5.png)

   做得很棒！

>[!TIP]
>
>使用“[!UICONTROL 请求营销活动]”触发器或筛选器查找受此流程步骤影响的人员。
