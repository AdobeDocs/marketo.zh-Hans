...
除了列出的问题外，请考虑使用Marketo人工智能诊断和解决复杂的操作问题（CRM同步失败、webhook错误、电子邮件投放根本原因分析、字段不匹配），在您的帐户中执行审核（电子邮件投放能力、订阅中心合规性、智能营销活动审核、评分模型评估），以及从简报和模板（事件项目、多语言电子邮件活动、网络研讨会设置）加快项目创建。 其他功能旨在帮助进行人工智能辅助的潜在客户分类以及大规模的数据扩充、通过补救建议进行性能分析，以及引导式调试技术配置，例如Velocity脚本和生命周期模型。
...
### 处理潜在客户级别记录的工作流（标准B2B联系人字段）

* 潜在客户调查和故障排除 — 检查单个潜在客户字段值、活动历史记录和生命周期进度，以诊断潜在客户是否达到MQL或符合营销活动资格的原因
...
* AI会继承请求用户的现有Marketo Engage权限 — 它无法访问潜在客户记录、字段或程序，除非用户已可通过产品UI查看这些内容
* 调查工作流程和数据操作工作流程必然需要潜在客户级别的数据，因为用户明确要求人工智能对这些记录进行分析、分类或采取行动
* 希望限制潜在客户数据处理的用户可以通过现有的Marketo Engage角色和权限控制限制对工具调查工作流的访问，同时保留对结构和管理人工智能功能的完全访问权限
