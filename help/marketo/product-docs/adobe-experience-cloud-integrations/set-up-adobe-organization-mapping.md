...
为了与Adobe应用程序(如Audience Manager)、 B2B CDP Marketo连接器、 [!DNL Dynamic Chat]等，您必须首先在Marketo中输入Adobe IMS组织凭据。
...
>[!CAUTION]
>
>对于登记到AdobeBusiness Platform和Identity Management System的客户，与订阅关联的组织ID将被填充，并且是只读字段。
...
1. 如果您是 _非_ 已登录，会在新选项卡/窗口中显示弹出窗口。 登录到您的Adobe组织（此操作验证组织访问权限）。
