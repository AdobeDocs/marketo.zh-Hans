...
除了帐户列表之外，您还可以创建从公共CRM帐户视图生成的动态帐户列表。 CRM帐户视图是一组规则，在显示帐户时充当过滤器。 例如，您可以使用它查找行业代表医疗保健行业的客户 *和* 营收超过1亿美元。

![](assets/one.png)

>[!NOTE]
>
>在中构建智能列表和Web营销活动时，会自动提供在Marketo Target帐户管理中创建的帐户列表 [Web个性化](/help/marketo/product-docs/web-personalization/using-web-segments/web-segments.md).

## 创建新帐户列表 {#create-a-new-account-list}

1. 单击 **新** 下拉菜单并选择 **新建帐户列表**.
...
1. 创建帐户列表后，开始使用 [向其中添加指定帐户](/help/marketo/product-docs/target-account-management/target/named-accounts/add-an-existing-named-account-to-an-account-list.md)！
   ...
   >[!NOTE]
   >
   >Marketo将仅显示具有2,000个或更少指定帐户的帐户列表的分析。

## 创建新的动态帐户列表 {#create-a-new-dynamic-account-list}

1. 单击 **新** 下拉菜单并选择 **新建动态列表**.

   ![](assets/1.png)

1. 在对话框中，选择 **CRM帐户视图** 或从下拉列表中键入名称以搜索它。
...
1. 选择要重命名的帐户，单击 **帐户列表操作** 下拉菜单并选择 **重命名帐户列表**.
...
1. 输入新名称，然后单击 **重命名**.
   ...
   >[!NOTE]
   >
   >CRM帐户视图每8小时同步到动态帐户列表。 如果它们尚未同步，Marketo将在下一个周期中同步它们。

## 删除帐户列表 {#delete-an-account-list}

>[!NOTE]
>
>这些步骤对于帐户列表和动态帐户列表都是相同的。

1. 选择要删除的帐户，单击 **帐户列表操作** 下拉菜单并选择 **删除帐户列表**.
