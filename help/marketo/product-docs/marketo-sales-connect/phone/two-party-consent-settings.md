---
unique-page-id: 45416566
description: 了解Sales Connect呼叫记录的双方同意设置。 遵守需要获得同意才能进行记录的州法律。
title: 双方同意设置
exl-id: d0468ea1-3009-4190-ab9b-74c6fae955cb
feature: Marketo Sales Connect
source-git-commit: 15427eacd2fc42a02f6a4c59d9102bacba02e57b
workflow-type: tm+mt
source-wordcount: '148'
ht-degree: 6%

---

# 双方同意设置 {#two-party-consent-settings}

为了确保录制呼叫时遵守美国两党同意法，作为管理员，您可以启用自己选择的预先录制消息，在录制呼叫开始时播放。

>[!PREREQUISITES]
>
>在执行以下步骤之前，必须先[启用呼叫录制](/help/marketo/product-docs/marketo-sales-connect/phone/enable-call-recording.md)。

1. 单击设置图标并选择&#x200B;**[!UICONTROL Settings]**。

   ![](assets/one-1.png)

1. 在[!UICONTROL Admin Settings]下，单击&#x200B;**[!UICONTROL General]**。

   ![](assets/two-1.png)

1. 向下滚动到[!UICONTROL Sales Connect Phone Settings]并单击&#x200B;**[!UICONTROL Manage recording notice]**。

   ![](assets/three-1.png)

1. 单击 **[!UICONTROL Import Recording]**。

   ![](assets/four-1.png)

   >[!NOTE]
   >
   >仅支持Wav和MP3文件。 上传文件不能超过30秒。

1. 从硬盘中选择所需的音频文件。

   ![](assets/five.png)

1. 上传完成后，在文件管理器中选择dota（三个点），然后单击&#x200B;**[!UICONTROL Select as Consent Notice]**。 完成后，单击 **[!UICONTROL OK]**。

   ![](assets/six.png)

1. 单击切换可允许在开始录制呼叫时播放选定的消息。

   ![](assets/seven.png)
