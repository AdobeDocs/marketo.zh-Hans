...
>[!NOTE]
>
>若要了解如何启用任务同步，请签出[同步 [!DNL Sales Connect] 任务/提醒到 [!DNL Salesforce] 任务](/help/marketo/product-docs/marketo-sales-connect/crm/salesforce-integration/salesforce-sync-settings.md#sync-sales-connect-tasks-reminders-to-salesforce-tasks)。

启用任务同步设置后，用户将看到他们的提醒任务与[!DNL Salesforce]双向同步。 这意味着用户可以从[!DNL Salesforce]或[!DNL Sales Connect]中管理任务，并且确信系统将保持一致性。

## 提醒任务字段同步 {#reminder-task-field-sync}

![](assets/reminder-task-sync-with-salesforce-1.png)

以下是[!DNL Sales Connect]中的提醒任务字段及其通过双向任务同步支持的相应[!DNL Salesforce]字段的列表。

<table>
 <tr>
  <th>[!DNL Sales Connect] 任务字段</th>
 ...
 <tr>
  ...
  <td><p>显示任务的状态。 [!DNL Sales Connect]任务有两个状态，这些状态映射到[!DNL Salesforce]任务状态选取列表中的两个值。</p>
  <p>在[!DNL Sales Connect]中打开=未在[!DNL Salesforce]中启动。</p>
  <p>在[!DNL Sales Connect]内完成=在[!DNL Salesforce]内完成。</p>
  <p>[!DNL Salesforce]中的其他状态值将不会同步到[!DNL Sales Connect]。</p></td>
 </tr>
 <tr>
  <td>[!UICONTROL Priority]</td>
  <td>[!UICONTROL Priority]</td>
  <td><p>[!DNL Sales Connect] 优先级可以是“正常”或“高”，它们映射到[!DNL Salesforce]中的“正常”和“高”优先级值。</p>
  <p>[!DNL Salesforce]中的低优先级值将不会同步到[!DNL Sales Connect]。</p></td>
 </tr>
 <tr>
...
>[!NOTE]
>
>任务的主题字段可以在[!DNL Sales Connect]中更新，并且如果正在使用[活动详细信息自定义](/help/marketo/product-docs/marketo-sales-connect/crm/salesforce-integration/configure-salesforce-activity-detail-customization.md)设置中的`{{activity_subject}}`动态字段，则该更新将在相应同步任务的[!DNL Salesforce]主题字段中同步。 相反，对[!DNL Salesforce]中的主题字段所做的任何更新将&#x200B;*不会*&#x200B;同步到[!DNL Sales Connect]提醒任务主题字段。
