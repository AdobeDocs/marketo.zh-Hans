...
>[!NOTE]
>
>要了解如何启用任务同步签出 [将Sales Connect任务/提醒同步到Salesforce任务](/help/marketo/product-docs/marketo-sales-connect/crm/salesforce-integration/salesforce-sync-settings.md#sync-sales-connect-tasks-reminders-to-salesforce-tasks).

启用任务同步设置后，用户将看到他们的提醒任务与Salesforce进行双向同步。 这意味着用户可以从Salesforce或Sales Connect管理任务，并且确信系统能够保持一致性。

## 提醒任务字段同步 {#reminder-task-field-sync}

![](assets/reminder-task-sync-with-salesforce-1.png)

以下是Sales Connect中通过双向任务同步支持的提醒任务字段及其对应的Salesforce字段的列表。
...
<table>
 ...
 <tr>
  <td>状态</td>
  <td>任务状态</td>
  <td><p>显示任务的状态。 Sales Connect任务有两种状态，这些状态映射到Salesforce任务状态选取列表中的两个值。</p>
...
>[!NOTE]
>
>任务的subject字段可在Sales Connect中更新，而且该更新将在Salesforce subject字段中针对相应的已同步任务进行同步(如果您使用的是 `{{activity_subject}}` 中的动态字段 [活动详细信息自定义](/help/marketo/product-docs/marketo-sales-connect/crm/salesforce-integration/configure-salesforce-activity-detail-customization.md) 设置。 相反，对Salesforce中的subject字段所做的任何更新将 _非_ 同步到Sales Connect提醒任务主题字段。
