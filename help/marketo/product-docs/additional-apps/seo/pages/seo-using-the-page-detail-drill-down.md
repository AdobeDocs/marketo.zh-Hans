---
unique-page-id: 2949186
description: 查看详细的页面信息，包括优化得分、入站链接、目标关键字和可操作的页面优化结果。
title: SEO - 使用页面详情深入分析
exl-id: 2fa1fdf3-2d42-424c-911e-188195e4551a
feature: SEO
source-git-commit: b42922726bebab913436bb0955557339af96d0f5
workflow-type: tm+mt
source-wordcount: '278'
ht-degree: 6%

---

# SEO - 使用页面详情深入分析 {#seo-using-the-page-detail-drill-down}

您可以使用页面详细信息向下钻取查看有关页面的许多信息。

>[!IMPORTANT]
>
>2026年3月31日，Marketo Engage将弃用搜索引擎优化功能。 请在3月30日或之前导出任何相关数据。 [了解详情](https://nation.marketo.com/t5/product-blogs/marketo-engage-seo-feature-deprecation/ba-p/359060){target="_blank"}。
>
>* [导出问题](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/additional-apps/seo/pages/seo-export-issues-to-csv){target="_blank"}
>* [导出关键字结果](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/additional-apps/seo/keywords/seo-exporting-keyword-results){target="_blank"}
>* [导出关键词趋势](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/additional-apps/seo/reports/seo-use-the-keyword-trends-report#exporting-data){target="_blank"}
>* [导出竞争者关键词趋势](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/additional-apps/seo/reports/seo-use-the-competitor-kw-trends-report#exporting-data){target="_blank"}

## 查找页向下展开 {#find-page-drill-down}

1. 转到&#x200B;**[!UICONTROL Pages]部分**。

   ![](assets/image2014-9-17-21-3a54-3a53.png)

1. 单击要查看其详细信息的页面。

   ![](assets/image2014-9-17-21-3a54-3a58.png)

   请注意，页面详细信息向下展开！

   ![](assets/image2014-9-17-21-3a55-3a2.png)

## [!UICONTROL Page Detail]节 {#page-detail-section}

![](assets/image2014-9-17-21-3a55-3a46.png)

| 项目 | 描述 |
|---|---|
| [!UICONTROL Optimization Score] | 秘汁用于向您展示您的页面在1到100之间优化搜索的力度。 |
| [!UICONTROL To Do]框 | 显示与您所在的页面有关的未完成问题的数量。 |
| [!UICONTROL Inbound Links Total] | 您网站的入站链接总数。 |
| [!UICONTROL Inbound Links (unique domains)] | 至少具有一个指向您网站的入站链接的其他网站数。 |
| [!UICONTROL Outbound Links] | 从此特定页面转到其他网站的链接数。 |
| [!UICONTROL Facebook Shares] | 在[!DNL Facebook]上共享此页面的次数。 |
| [!UICONTROL Facebook Likes] | 此页面共享在[!DNL Facebook]上的点赞次数。 |
| [!UICONTROL Facebook Clicks] | 来自[!DNL Facebook share]的点击次数。 |

## [!UICONTROL Keyword(s) that Page is Optimized For]  {#keyword-s-that-page-is-optimized-for}

在此处，您[选择您尝试为](/help/marketo/product-docs/additional-apps/seo/keywords/seo-optimize-specific-pages-with-targeted-keywords.md)优化页面的关键字。

>[!NOTE]
>
>这是充分利用此功能的重要步骤。 我们建议您每页使用的关键字数少于三个。 最好只有一个。

![](assets/image2014-9-17-21-3a56-3a35.png)

## [!UICONTROL Page Optimization Results] {#page-optimization-results}

[!UICONTROL Page Optimization Results]将告诉您需要执行的确切步骤，以提高您的优化得分/更好地搜索此页面。

![](assets/image2014-9-17-21-3a56-3a41.png)

>[!MORELIKETHIS]
>
>[使用目标关键字优化特定页面](/help/marketo/product-docs/additional-apps/seo/keywords/seo-optimize-specific-pages-with-targeted-keywords.md)
