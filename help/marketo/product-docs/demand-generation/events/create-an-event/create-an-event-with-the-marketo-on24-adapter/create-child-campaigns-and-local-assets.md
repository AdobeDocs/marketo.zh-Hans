...
要确保用户正确注册了ON24，您的Marketo表单中必须包含以下字段：
...
向注册活动添加适当的流程步骤后，用户将被推送到ON24并标记为已注册。 您可以将其他字段添加到表单，并且信息将作为人员详细信息记录的一部分捕获到Marketo中。

>[!CAUTION]
>
>要成功集成，您必须使用Marketo表单注册人员以使用事件，或使用具有正确API集成的非Marketo表单将注册数据推送到Marketo。
...
>[!TIP]
>
>您可以配置ON24以发送确认、提醒或跟进电子邮件。 请参阅 [ON24帮助站点](https://www.on24.com/live-webcast-elite/){target="_blank"} 了解更多信息。

## 注册子营销活动要求 {#registration-child-campaign-requirements}

事件包含一个或多个子营销策划，这些子营销策划可共同推动人员浏览项目状态，并让您跟踪事件的效果。

子营销活动的示例包括邀请营销活动、注册营销活动和跟进营销活动。

>[!CAUTION]
>
>要使适配器完成其工作，您必须创建一个注册活动。 此营销活动必须由填写表单的人员触发，并且第一步必须将人员的项目状态更改为 **已注册**. 然后，营销活动会发送一封确认电子邮件。 有关详细信息，请参阅本文的其他部分。

**注册/确认（触发营销活动）**

* 智能列表
* 触发器依据 **填写表单**. 确保包括表单所在的登陆页面，请使用 **添加约束**，尤其是在多个登陆页面上使用相同表单时。

>[!CAUTION]
>
>您必须使用Marketo表单注册该事件的人员，或使用具有正确API集成的非Marketo表单将注册数据推送到Marketo。 这对于成功集成活动合作伙伴至关重要。

>[!NOTE]
>
>如果您在非Marketo登录页面上使用Marketo表单，则您的触发器将为 **填写表单** 表单名称。
...
在设置子营销策划时，此流程步骤作为第一个流程步骤是必需的。 当人员的计划状态变为“已注册”时，Marketo会将注册信息推送到ON24。 没有其他状态会将人员推到。

* **发送电子邮件**  — 确认电子邮件。 将此电子邮件设置为 **可操作** 这样那些注册了的非订阅者就仍然可以接收到。
...
>[!NOTE]
>
>这些流程步骤的顺序非常重要，因为在Marketo中执行操作的顺序很重要。 此 **更改项目状态** 步骤将用户发送到ON24进行注册，并生成一个唯一的URL。 之后，您可以使用发送包含此唯一URL的确认电子邮件 `{{member.webinar URL}}` 令牌。
>
