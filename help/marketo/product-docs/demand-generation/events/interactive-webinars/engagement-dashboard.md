...
仪表板跟踪各种量度，包括投票回复、聊天活动和视频交互，为参与者的偏好和行为提供有价值的分析。 它全面概述了参与者的活动和参与级别，允许主持人监控实时出席情况、参与情况和交互情况。
...
此界面提供活动实时会话性能的概述。 该概述包括关键量度、一段时间内的参与度和参与者交互。 它有助于组织者评估活动是否成功并确定需要改进的方面。

从左侧面板中选择参与以查看实时会话的性能。 选择参与摘要(PDF)以下载实时会话的性能摘要。 该概述分为不同的部分。

屏幕快照

**关键量度**

从关键量度查看实时会话中的注册数、出席人数和参与度。
...
通过参与度图表了解实时会话期间一段时间的平均参与度。 主机可以监测参与度水平如何波动，从而识别高交互或低交互的关键时刻。 检查不同布局的参与参与者获得了多少平均参与度分数。
...
选择转到交互可查看轮询响应、问题与解答的量度以及与会者在会话期间的反应。 从交互中查看与会者在Pod中的参与情况，并从以下部分下载交互报表。

### 交互 {#interactions}

通过交互查看与会者如何交互并参与会话。 跟踪对轮询、QnA量度、与会者的反应以及相应Pod中删除的链接和文件的响应。 主机还可以下载这些Pod的交互报表，以便更好地进行分析。 通过分析这些互动，找出趋势并调整其策略，以鼓励营造更具互动性和更引人入胜的环境。

从左侧面板中选择交互以查看与会者如何跨pod参与。

屏幕快照

从菜单栏中选择投票选项卡以查看添加到投票的问题和响应。 此选项卡显示以下信息：

* 选择轮询报表(CSV)可下载轮询面板中交互的报表。
* 轮询问题及其类型。
* 响应数量和轮询面板打开和参与的时段。
* 选择“查看所有`<number>`响应”可在对话框窗口中查看它们。

从菜单栏中选择其他交互选项卡，查看其他Pod中的参与并下载其报表。

屏幕快照

从右上角的下拉菜单中选择下载交互报表，可下载不同Pod的报表。 从此处下载问题与解答报表、链接和文件报表以及反应报表，以便更好地跟踪。
...
从左侧面板中选择按需活动以查看录制摘要。 您还可以下载按需活动报表。
...
从左侧面板中选择“参与者活动”以查看有关每个参与者的参与级别的综合信息。 查看如何从参与者参与级别将此参与分类。 选择用户活动报表(CSV)以下载报表以进行更好的跟踪。
...
企业使用Adobe Connect提供高度定制、品牌化和以参与为主导的网络研讨会。 投票、聊天、问答和反应等交互式工具有助于吸引与会者，并且收集不仅仅是注册和出席数据。 在与会者使用这些交互功能后，可使用参与数据将与会者分为三个参与级别：高、中和低。 营销人员可以使用参与级别更新受众区段、更新潜在客户得分和提醒销售。
