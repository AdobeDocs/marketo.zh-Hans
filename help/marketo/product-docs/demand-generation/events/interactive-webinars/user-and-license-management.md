...
已获得交互式网络研讨会权限的Marketo用户，他们是 _非_ 特定交互式网络研讨会活动计划的所有者将能够对这些计划执行有限的职能。 Marketo的非管理员用户将能够访问和克隆程序，但如果他们具有交互式网络研讨会的权限，则将无法执行任何其他功能。 但是，Marketo管理员用户 _将_ 能够执行所有功能，如访问、修改、克隆、移动和删除该程序（只要他们有权参加交互式网络研讨会）。 在撤销对Marketo管理员和非管理员用户的此权限后，他们将只能访问交互式网络研讨会事件计划，而不能执行任何其他功能。
