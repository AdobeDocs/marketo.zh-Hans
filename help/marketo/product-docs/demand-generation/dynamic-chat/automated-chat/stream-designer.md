---
description: 了解Stream Designer和用于构建聊天对话的卡片。 将消息、问题、文档捕获、会议预订等添加到对话框。
title: 流设计器
...
<table>
 ...
 <tr>
  ...
  <td>这是访客不会看到的唯一一张卡。 由您确定在特定聊天中实现目标的时间（例如：如果收集访客的电子邮件是您的目标，则将“目标”卡片放置在流中信息捕获后的紧跟位置）。</td>
 </tr>
 <tr>
...
在Stream Designer的右上角，您会看到几个图标。 下面描述了每个图标。
...
您可以为对话框或[对话Forms](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/conversational-flow-overview.md){target="_blank"}创建流。 在此示例中，为对话框创建流。
...
1. 对于“否”，将拖到该选项下方的另一个[!UICONTROL Question]卡片上，以检查访客是否想要加入邮件列表。

   ![](assets/stream-designer-14.png)

1. 输入您的响应，并为访客添加响应选项。 完成后，单击 **[!UICONTROL Save]**。
...
1. 输入聊天机器人消息和占位符。 如果数据库中已存在此属性的值，请选择是要跳过还是让此数据覆盖它。 完成后，单击 **[!UICONTROL Save]**。
...
1. 如果回答“否”，请记得添加响应。 一种选择是将信息卡拖到下面并说“谢谢”。 但在此示例中，将改为提供免费的PDF文档。

   ![](assets/stream-designer-21.png)

1. 创建新文档。 为其命名，输入已托管的PDF的URL，然后单击&#x200B;**[!UICONTROL Save]**。
...
>[!NOTE]
>
>在单击[!UICONTROL Publish]之前，请确认您已[输入目标URL](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/audience-criteria.md#target){target="_blank"}。
