---
description: 了解如何使用源URL和主题为Dynamic Chat生成问题。 创建人工智能辅助响应并在辅助响应中跟踪任务状态。
title: 问题生成
...
1. 为您的任务提供一个名称并输入源URL（最多10个），将从其中提取所有内容。 输入所需的主题/关键字，然后在键盘上按Enter键。 完成后，单击&#x200B;**[!UICONTROL Generate]**。
   ...
   >[!IMPORTANT]
   >
   >要确保Marketo Engage能够从提供的URL中擦除内容，您必须先多个IP地址。 [有关详细信息，请参阅下文](#ip-addresses-to-allowlist)。
...
1. 在Excel文件中，**[!DNL Task details]**&#x200B;显示有关该任务的各种详细信息，包括有关如何添加或编辑问题和响应的说明。
...
为了在生成问题和响应期间从Web URL提取内容，请在下面找到您所在的区域，并验证与其关联的IP地址是否由您的Web团队列入允许列表。
