---
description: 了解Dynamic Chat构件回调函数，以便将对话事件发送到第三方平台。 使用事件执行已触发、参与和达到目标的操作。
title: 回调函数
...
如果您希望将任意这些事件发送到Adobe Analytics或Google Analytics等Analytics平台，则需要在这些Dynamic Chat事件中添加各自的跟踪调用。 它类似于下面的示例。
