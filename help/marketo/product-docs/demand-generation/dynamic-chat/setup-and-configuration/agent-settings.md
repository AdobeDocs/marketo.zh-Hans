---
description: 了解如何在Dynamic Chat中配置代理日历和会议或实时聊天可用性。 连接Outlook或Gmail以进行约会计划。
title: 代理设置
...
>[!PREREQUISITES]
>
>验证您的代理是否已被授予适当的[权限](/help/marketo/product-docs/demand-generation/dynamic-chat/setup-and-configuration/permissions.md){target="_blank"}。
...
如果您已登录应用程序，将会收到传入聊天的应用内通知。 如果您未登录，则将收到浏览器通知（如果您已[设置](/help/marketo/product-docs/demand-generation/dynamic-chat/live-chat/agent-inbox.md#live-chat-notifications){target="_blank"}）。

>[!IMPORTANT]
>
>* 代理收件箱&#x200B;**中的[可用性切换](/help/marketo/product-docs/demand-generation/dynamic-chat/live-chat/agent-inbox.md#availability-toggle){target="_blank"}将覆盖**&#x200B;在&#x200B;_实时聊天可用性_&#x200B;选项卡中输入的内容。 因此，如果座席从1p安排为5p可用，但需要在3p进行快速中断，则无需更改其座席设置。 可用性切换状态将一直保持到手动更改，直到到达代理可用性的下一个时间块，或者直到代理指定时区的午夜（有关更多信息，请参阅下一个项目符号）。
>
