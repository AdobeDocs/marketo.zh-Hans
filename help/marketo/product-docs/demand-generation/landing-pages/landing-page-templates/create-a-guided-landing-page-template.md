...
引导式登陆页面模板具有特殊语法。 使用此语法指定可自定义的内容以及内容在从模板构建的每个登陆页面上的结束位置。 只有指定为可编辑的区域或变量才可以在“引导式”登陆页面编辑器中自定义。
...
* 将对象声明为“element”。 登陆页面创建者将能够将图像、文本或Marketo资源添加到这些指定的区域。
* 将字符串声明为“变量”。 登陆页面创建者将能够通过true/false杠杆将该变量替换为字符串、颜色或布尔状态。

## 可编辑元素 {#editable-elements}

通过以下方法声明元素：向模板中添加标准DOM元素，然后使用Marketo特定的类名称修饰该元素。

## 文本 {#text}

如果将区域定义为富文本，用户将能够编辑其内容 [使用Marketo富文本编辑器](/help/marketo/product-docs/email-marketing/general/understanding-the-email-editor/using-the-rich-text-editor.md).
...
有两个选项可用于定义可编辑的图像元素。 您可以使用 `<div>`，指定图像将插入到的容器，或者 `<img>` 标记之前。
...
>[!NOTE]
>
>使用时 `<img>` 版本，渲染的HTML将包含 `<img>` 标记之前。 它将设置为类。”mktoImg.mktoGen”，和将为display：inline-block。
...
所有变量类型都可通过引用包装在${ }字符序列中的id属性的值来使用。 它们可以在文档中的任意位置使用，但其他变量声明的内部除外。
...
变量在中声明为元标记 `<head>` 模板元素。 可以使用三种类型的变量：字符串、颜色和布尔值。
...
可选：\
**默认**：布尔字符串。 “true”或“false”控制值是否从ON或OFF位置开始。 &quot;false&quot;（如果未提供）。\
**false_value**：字符串。 变量处于OFF位置时要插入的值。 &quot;false&quot;（如果未提供）。\
**true_value**：字符串。 变量处于ON位置时要插入的值。 如果未提供，则为“true”。\
**false_value_name**：字符串。 当值处于OFF位置时将在登陆页面编辑器中显示的显示名称。 如果未提供，则为“OFF”。\
