...
description: 了解工作区如何组织营销资产以及人员分区如何充当单独的数据库。
title: 了解工作区和人员分区
...
>[!CAUTION]
>
>工作区的设置可能较为复杂。 联系[Marketo支持](https://nation.marketo.com/t5/Support/ct-p/Support)以了解他们是否适合您。
...
>[!NOTE]
>
>了解如何[创建新工作区](/help/marketo/product-docs/administration/workspaces-and-person-partitions/create-a-new-workspace.md)。

## 跨工作区共享 {#sharing-across-workspaces}

以下步骤说明了如何在工作区之间共享资源。 该方法适用于所有可共享的内容；此示例以分段为例。
...
1. 选择要共享该文件夹的工作区，然后点击 **[!UICONTROL Save]**。 “共享文件夹”对话框只会显示您有权限查看的工作区。
   ...
   >[!NOTE]
   >
   >现在，原始文件夹将有一个绿色的小箭头，指示该文件夹已共享。 在共享的工作区中，该文件夹将显示一个锁定图标，表示为只读。
...
对于不是模板的资产，建议在项目中将其克隆为本地资产。 在具备相应访问权限的情况下，您可以将以下资源拖放到另一个工作区中：
...
>[!NOTE]
>
>包含成员的程序不能从一个工作区移动到另一个工作区。

## 人员分区 {#person-partitions}

人员分区的作用类似于独立的数据库。 每个分区都有自己独立的人员数据，不会与其他分区进行去重或混合。 如果您的业务用例需要具有相同电子邮件地址的重复记录，请联系[Marketo支持](https://nation.marketo.com/t5/Support/ct-p/Support)。
