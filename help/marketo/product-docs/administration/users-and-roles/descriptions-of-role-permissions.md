...
* 访问Adobe组织映射 — 赋予用户访问Adobe组织映射屏幕的权限
* 访问管理员审核记录 — 授予用户访问管理员审核记录屏幕的权限
* 访问审核记录 — 赋予用户访问审核记录的权限
...
* 访问Predictive Audiences — 让用户可以访问Predictive Audiences屏幕
...
* 访问Webhook — 让用户可以访问Admin中的Webhook，以设置详细信息和响应映射
...
* 创建报表 — 赋予用户在Analytics和Marketing Activities以及收入周期Modeler资产中创建、克隆、读取、更新和移动报表资产的权限
