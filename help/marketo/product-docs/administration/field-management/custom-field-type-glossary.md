...
在Marketo中创建自定义字段时，您有一个类型列表可供选择。
...
>[!NOTE]
>
>大多数字段的最大字符数并不是最多，而是字节。 因此，Marketo无法为每个字段提供明确的字符限制。 异常为&#x200B;**字符串**，其长度上限为255个字符。

## 布尔值 {#boolean}

**示例名称：**&#x200B;为Customer — 将人员标记为客户
...
**示例名称：**&#x200B;备用电子邮件 — 为联系人保留备用电子邮件地址（实际上无法像默认电子邮件地址字段那样向此字段发送电子邮件）
