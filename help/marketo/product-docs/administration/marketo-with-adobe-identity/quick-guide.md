---
description: 快速指南 — Marketo文档 — 产品文档
title: 快速指南
hide: true
hidefromtoc: true
source-git-commit: f33af70109b0491138aff8aa0416f3f919e7d20c
workflow-type: tm+mt
source-wordcount: '0'
ht-degree: 0%

---

# 快速指南 {#quick-guide}

下面列出的文章将帮助您设置并了解有关Marketo Engage的AdobeIdentity Management。

## 系统管理员 {#system-admin}

产品配置文件和产品管理员创建。

[系统管理员访问权限](/help/marketo/product-docs/administration/marketo-with-adobe-identity/admin-setup.md#system-admin-setup){target=&quot;_blank&quot;}

[创建产品配置文件](/help/marketo/product-docs/administration/marketo-with-adobe-identity/admin-setup.md#create-a-product-profile){target=&quot;_blank&quot;}

[添加产品管理员](/help/marketo/product-docs/administration/marketo-with-adobe-identity/add-or-remove-a-product-admin.md#add-a-product-admin){target=&quot;_blank&quot;}

## 用户管理 {#user-management}

添加和删除用户。 由产品管理员（或系统管理员）完成。

[添加用户](/help/marketo/product-docs/administration/marketo-with-adobe-identity/add-or-remove-a-user.md#add-a-user){target=&quot;_blank&quot;}

[删除用户](/help/marketo/product-docs/administration/marketo-with-adobe-identity/add-or-remove-a-user.md#remove-a-user){target=&quot;_blank&quot;}

## API中的仅用户Marketo Engage {#api-only-user-in-marketo-engage}

Marketo EngageAPI必须在Marketo Engage中创建和管理仅用户。

[为启用了Adobe IMS的订阅添加仅API用户](/help/marketo/product-docs/administration/marketo-with-adobe-identity/add-api-only-user-for-adobe-ims-enabled-subscriptions.md){target=&quot;_blank&quot;}

## 通过CSV添加用户 {#add-users-via-csv}

[导入CSV](/help/marketo/product-docs/core-marketo-concepts/programs/working-with-programs/import-members-from-a-spreadsheet-into-a-program.md){target=&quot;_blank&quot;}

## 用户登录 {#user-sign-in}

[用户使用Adobe ID登录](/help/marketo/product-docs/administration/marketo-with-adobe-identity/user-sign-in-with-adobe-id.md){target=&quot;_blank&quot;}

## 常见问题解答 {#faq}

[常见问题解答](/help/marketo/product-docs/administration/marketo-with-adobe-identity/overview.md#faq){target=&quot;_blank&quot;}
