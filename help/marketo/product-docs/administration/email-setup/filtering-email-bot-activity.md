---
description: 使用IAB列表匹配和邻近模式过滤或记录电子邮件机器人活动，以防止打开次数和点击次数夸大。
title: 筛选电子邮件机器人活动
exl-id: 70c97159-72bf-46e5-b29b-247615d0fa80
feature: Email Setup
TQID: https://experienceleague.adobe.com/b7H7jXcwtzD4UHkNkDoRWumbkR35U45VuDayeaWnXho
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2:
  - id: d1d0a9cd-295d-4976-8c39-ddae266f240e
topic_v2:
  - id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 469
ht-degree: 0%

---

# 筛选电子邮件机器人活动 {#filtering-email-bot-activity}

有时，电子邮件机器人活动可能会错误地夸大电子邮件打开数和点击数数据。 请按照以下步骤解决此问题。

使用两种单独的方法确认机器人活动：

* 与[Interactive Advertising Bureau机器人列表](https://www.iab.com/guidelines/iab-abc-international-spiders-bots-list/){target="_blank"}匹配：与IAB UA/IP（用户代理/IP地址）列表中的任何内容匹配的活动将被标记为机器人。
* 与邻近模式匹配：当两个或多个活动同时发生（在一秒之内）时，它们将被识别为机器人。 比较过程中考虑的属性包括：
   * 商机ID（应相同）
   * 电子邮件资源（应相同）
   * 链接点击或电子邮件打开
   * 时间差（应小于1秒）

针对电子邮件链接点击和电子邮件打开活动，新属性将填充以下值：

* 被标识为机器人的活动将具有“机器人活动”作为“真”，“机器人活动模式”作为被标识的模式/方法
* 被识别为非机器人的活动将具有“机器人活动”作为“False”，“机器人活动模式”作为“N/A”
* 在引入这些属性之前发生的活动将“机器人活动”作为“ ”（空），“机器人活动模式”作为“ ”（空）

## 选择筛选器类型 {#select-filter-type}

1. 单击&#x200B;**[!UICONTROL Admin]**。

   ![](assets/filtering-email-bot-activity-1.png)

1. 单击&#x200B;**[!UICONTROL Email]**。

   ![](assets/filtering-email-bot-activity-2.png)

1. 单击&#x200B;**[!UICONTROL Bot Activity]**&#x200B;选项卡。

   ![](assets/filtering-email-bot-activity-3.png)

1. 有两个滑块可供选择。 您可以只启用一个或同时启用两者。 如果启用&#x200B;**[!UICONTROL Match with IAB List]**，请选择是[!UICONTROL log bot activity] _还是_ [!UICONTROL filter bot activity]。

   ![](assets/filtering-email-bot-activity-4.png)

1. 如果启用&#x200B;**[!UICONTROL Match with Proximity Pattern]**，请选择是[!UICONTROL log bot activity] _还是_ [!UICONTROL filter bot activity]。 您还可以设置&#x200B;**Duration Between Activities**&#x200B;的秒数（默认值为0，最大值为3）。

   ![](assets/filtering-email-bot-activity-5.png)

>[!NOTE]
>
>如果将&#x200B;**Duration Between Activities**&#x200B;设置为0秒，则将电子邮件活动识别为在同一秒发生。 如果在指定的秒数内发生了多个电子邮件活动，则会将其识别为机器人活动。

>[!IMPORTANT]
>
>* 如果选择[!UICONTROL Filter Bot Activity]，您可能会看到电子邮件打开次数和点击次数出现下降，因为虚假活动会被删除。

**可选步骤**：要禁用任一功能，请取消选择相应的滑块。 如果这样做，数据将不会重置。

>[!TIP]
>
>通过“电子邮件中的点击链接”和“打开电子邮件”过滤器中的“是机器人活动”布尔值（是/否）和“机器人活动模式”，以及“电子邮件中的点击链接”和“打开电子邮件”触发器，在智能列表中使用机器人活动数据。

## IP阻止列表 {#ip-blocklist}

Marketo编制了一份IP地址列表，这些地址负责生成数百万次虚假参与。 因此，从以下IP收到的任何参与都会自动过滤掉，而不会添加到您的Marketo Engage订阅。 这可能会导致电子邮件打开次数、点击次数和其他相关活动减少。 以下列表可能会定期更新。

* 40.94.34.52
* 40.94.34.86
* 52.34.76.65
* 54.70.53.60
* 54.71.187.124
* 60.28.2.248
* 64.235.150.252
* 64.235.153.10
* 64.235.153.2
* 64.235.154.105
* 64.235.154.109
* 64.235.154.140
* 64.74.215.1
* 64.74.215.100
* 64.74.215.138
* 64.74.215.139
* 64.74.215.142
* 64.74.215.146
* 64.74.215.150
* 64.74.215.154
* 64.74.215.158
* 64.74.215.162
* 64.74.215.164
* 64.74.215.166
* 64.74.215.170
* 64.74.215.174
* 64.74.215.176
* 64.74.215.178
* 64.74.215.51
* 64.74.215.56
* 64.74.215.58
* 64.74.215.59
* 64.74.215.86
* 64.74.215.98
* 65.154.226.101
* 66.249.91.149
* 70.42.131.106
* 74.125.217.116
* 74.217.90.250
* 104.129.41.4
* 104.47.55.126
* 104.47.58.126
* 104.47.70.126
* 104.47.73.126
* 104.47.73.254
* 104.47.74.126
* 128.220.160.1
* 155.70.39.101
* 162.129.251.14
* 162.129.251.42
* 208.52.157.204

>[!NOTE]
>
>每个IP地址在添加到此列表之前都经过仔细检查，确保仅阻止危害最大的IP。
