---
unique-page-id: 2359902
description: 管理员如何查看和编辑订阅的默认位置设置，包括语言、区域设置、时区和货币。
title: 为订阅设置默认位置
exl-id: 9181e22b-be60-4dc0-bc75-f4583ee5dbea
feature: Administration
source-git-commit: 7e9e062fba3361e5b40830ebda16c794d236a5ff
workflow-type: tm+mt
source-wordcount: '233'
ht-degree: 12%

---

# 为订阅设置默认位置 {#set-default-location-settings-for-a-subscription}

本文介绍了管理员如何查看和编辑订阅的默认位置设置，包括语言、区域设置和时区。

>[!NOTE]
>
>需要管理员权限。 该语言通常不是管理员会更改的内容。 它是在购买时指定的，这样就可以以相应的语言生成订阅。

## 为订阅设置默认位置 {#set-default-location-settings-for-a-subscription-1}

当管理员更改默认位置设置时，新创建的用户将继承这些设置。 用户可以在个人帐户中[更改其时区](/help/marketo/product-docs/administration/settings/change-time-zone.md)。

1. 进入 **[!UICONTROL Admin]** 区域。

   ![](assets/set-default-location-settings-for-a-subscription-1.png)

1. 单击 **[!UICONTROL Location]**。

   ![](assets/set-default-location-settings-for-a-subscription-2.png)

1. 单击 **[!UICONTROL Edit]**。

   ![](assets/set-default-location-settings-for-a-subscription-3.png)

   此订阅使用英语创建。 假设您位于伦敦，希望更改默认区域设置和时区。 区域设置决定了数字、日期和时间的格式。

1. 选择&#x200B;**[!UICONTROL Locale]**&#x200B;并将其更改为&#x200B;**[!UICONTROL English (United Kingdom)]**。

   ![](assets/set-default-location-settings-for-a-subscription-4.png)

1. 最后，选择适当的&#x200B;**[!UICONTROL Time Zone]**。

   ![](assets/set-default-location-settings-for-a-subscription-5.png)

   >[!NOTE]
   >
   >Marketo Sales Insight for [Salesforce.com](https://salesforce.com/)支持法语、德语、日语、葡萄牙语和西班牙语。

## 设置订阅的默认货币设置 {#set-the-default-currency-settings-for-a-subscription}

如果要更改用户的默认区域设置，则可能还需要更改货币格式设置。

1. 在&#x200B;**[!UICONTROL Edit]**&#x200B;中单击[!UICONTROL Subscription Currency Settings]。

   ![](assets/set-default-location-settings-for-a-subscription-6.png)

1. 选择您选择的货币格式并单击&#x200B;**[!UICONTROL Save]**。

   ![](assets/set-default-location-settings-for-a-subscription-7.png)

   恭喜！您已更改订阅的位置设置。

>[!MORELIKETHIS]
>
>* [更改您的时区](/help/marketo/product-docs/administration/settings/change-time-zone.md)
