...
您可以编辑域名和备用页面，启用或禁用表单预填充，防止滥用登陆页面等。 请按照以下步骤操作。
...
1. 输入您的域和页面信息。
   ...
   | 术语 | 定义 |
   |---|---|
   | [!UICONTROL Domain name for landing pages] | 这是您的CNAME。 CNAME是您为登陆页面授予用户的URL的第一部分。 例如，在`https://go.yourCompany.com`中，“go”一词就是CNAME。 您可以有多个，但大多数人只能使用一个。 |
