...
1. 命名链接字段并添加可选[!UICONTROL Description]。 选择[!UICONTROL Link]数据类型。

   ![](assets/add-marketo-custom-object-link-fields-5.png)

   >[!CAUTION]
   >
   >在自定义对象获得批准后，无法返回创建、编辑或删除[!UICONTROL Link]或[!UICONTROL Dedupe Field]。
...
1. 创建两个链接字段，一次一个。 首先，将数据库列表成员的字段命名为（例如，leadID）。 添加可选[!UICONTROL Description]。 选择[!UICONTROL link] [!UICONTROL Data Type]。
...
下一步是在智能营销策划的过滤器中使用这些自定义对象。 通过多对多关系，您可以选择多个人员或公司以及多个自定义对象。 在下面的示例中，将列出数据库中符合这些条件的所有人员。 课程名称字段来自课程自定义对象，注册级别来自中间对象。
