...
description: 向自定义对象添加字段（包括显示名称、描述和数据类型）的步骤。
...
字段定义自定义对象使用的特定信息。 链接字段用于特定目的（连接自定义对象），并包含在[单独的文章](/help/marketo/product-docs/administration/marketo-custom-objects/add-marketo-custom-object-link-fields.md)中。
...
1. 输入[!UICONTROL Display Name]和（可选） [!UICONTROL Description]。
   ...
   >[!NOTE]
   >
   >[!UICONTROL API Name]只有获得批准才能进行编辑。

1. 从列表中选择适当的[!UICONTROL Data Type]。
...
1. 添加您需要的任何其他字段。
   ...
   >[!NOTE]
   >
   >如果您构建一对多结构，则需要将链接字段添加到自定义对象。 对于多对多结构，自定义对象中不需要链接字段，但必须在中间对象中添加两个链接字段。 有关自定义对象类型的更多信息，请参阅[添加Marketo自定义对象链接字段](/help/marketo/product-docs/administration/marketo-custom-objects/add-marketo-custom-object-fields.md)以创建链接字段，以及[了解Marketo自定义对象](/help/marketo/product-docs/administration/marketo-custom-objects/understanding-marketo-custom-objects.md)。
