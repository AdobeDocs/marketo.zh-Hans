...
将自定义对象用作智能营销活动中的筛选器和触发器。 例如：

* **筛选条件**：仅向特定车辆品牌的所有者发送电子邮件
* **触发器**：将自定义对象添加到人员或公司时发送电子邮件。

您可以在一对多或多对多关系中设置自定义对象。 例如：

* **一对多**：一人拥有多辆汽车
* **多对多**：多名学生从课程目录注册了多个课程

一对多结构使用单个链接字段将自定义对象连接到人员或公司。

多对多的自定义对象使用两个链接字段，这些字段是中间对象的一部分。 一个“链接”字段连接到人员或公司，另一个“链接”字段连接到自定义对象，如课程目录。 此中间对象可以包含其他自定义字段，如课程级别或出勤日期，这些字段进一步定义了连接的性质。
...
>[!CAUTION]
>
>您无法恢复自定义对象，因此在删除它们之前，请确保不再需要它们。
...
1. Marketo自定义对象显示将在右侧列出所有自定义对象，但只列出主网格中批准的对象。
...
创建自定义对象结构后，当您上载特定的自定义对象数据时，自定义对象会自动使用自定义对象中的链接字段与数据库中的人员相关联。 您可以从以下位置查看信息： [!UICONTROL 自定义对象] 选项卡。

1. 转到 **[!UICONTROL 数据库]**.

   ![](assets/understanding-marketo-custom-objects-5.png)

1. 打开数据库并单击 **[!UICONTROL 人员]** 选项卡。 双击与自定义对象关联的人员的记录。
...
如果从CRM同步公司，或者使用API明确创建公司，则链接到公司的自定义对象效果最佳。 我们还建议您使用公司ID作为链接字段。

如果您在Marketo中有多个人员是CRM或仅限Marketo的记录中的记录，则链接到公司的自定义对象不会与多个单个记录关联。 这是因为只有当公司从CRM同步或者如果您使用API明确创建公司时，才支持在其下有多个人员的公司。

自定义对象只能直接链接到单个记录。 这意味着当您的自定义对象类型按公司字段链接时，如果您使用Marketo的REST API管理公司，则应确保通过CRM中的联系人转换或使用externalCompanyId字段将人员记录与公司关联。 对于未明确链接到公司记录的人员记录，使用公司链接的自定义对象将随机链接到单个记录，即使公司字段的值在多个人员之间共享。

参见 [导入自定义对象数据](/help/marketo/product-docs/administration/marketo-custom-objects/import-custom-object-data.md) 了解更多信息。
