...
>[!CAUTION]
>
>如果您为Dynamics Sync启用了多重身份验证(MFA)，则必须禁用它以便Dynamics能够与Marketo正确同步。 欲知更多信息，请联系 [Marketo支持](https://nation.marketo.com/t5/Support/ct-p/Support){target="_blank"}.
...
1. 单击 **[!UICONTROL 管理员]** 选项卡，然后 **[!DNL Microsoft Dynamics]** 集成区域中的链接。
...
1. 输入用户名、密码和URL（客户端ID和客户端密钥是可选的）。 单击 **[!UICONTROL 下一个]** 完成时。

   ![](assets/four-1.png)

   >[!NOTE]
   >
   >如果您之前已同步， **[!UICONTROL CRM]** 在左边的树中将显示 **[!DNL Microsoft Dynamics]**，并且可以预填充上述表单中的数据。

1. 如果一切正常，验证同步会生成一个充满绿色复选标记的核对清单 ![—](assets/check.png).

   ![](assets/image2015-9-22-15-3a58-3a12.png)

1. 如果您看到 ![—](assets/delete.png)，则该步骤有一个问题。 请参阅 [修复Dynamics验证同步问题](/help/marketo/product-docs/crm-sync/microsoft-dynamics-sync/sync-setup/validate-microsoft-dynamics-sync/fix-dynamics-validation-sync-issues.md){target="_blank"} 以识别和修复问题。 然后，重新运行同步验证步骤，直到结果类似于上图。
