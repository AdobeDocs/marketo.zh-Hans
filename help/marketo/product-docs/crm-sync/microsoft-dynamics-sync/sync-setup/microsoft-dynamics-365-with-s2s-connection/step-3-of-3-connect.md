...
>[!IMPORTANT]
>
>如果您要从基本身份验证升级到OAuth，您需要联系 [Marketo支持](https://nation.marketo.com/t5/support/ct-p/Support) 以获取有关更新其他参数的帮助。 启用此功能将暂时停止同步，直到输入新凭据并重新启用同步为止。 如果您希望恢复到旧的身份验证模式，可以禁用该功能（直到2022年4月）。
...
1. 输入Dynamics同步用户信息并单击 **保存** 完成时。
   ...
   >[!NOTE]
   >
   >Marketo中的用户名必须匹配 [电子邮件地址](https://docs.microsoft.com/en-us/power-platform/admin/manage-application-users#view-or-edit-the-details-of-an-application-user) CRM中应用程序用户的个人资料。 格式可以是 `user@domain.com` 或DOMAIN\user。
