...
>[!CAUTION]
>
>请注意“客户端密钥”值（见下面的屏幕快照），因为它只显示一次，无法再次检索。
...
1. 按照以下链接中的步骤操作[在Microsoft](https://docs.microsoft.com/en-us/powerapps/developer/common-data-service/use-single-tenant-server-server-authentication#application-user-creation){target="_blank"}中设置应用程序用户。
   ...
   >[!IMPORTANT]
   >
   >* 在为“应用程序用户”授予权限时，请将其分配给“Marketo同步用户角色”。
...
其他引用[可在此处](https://docs.microsoft.com/en-us/azure/active-directory/reports-monitoring/concept-all-sign-ins#:~:text=Interactive%20user%20sign%2Dins%20are,as%20the%20Microsoft%20Authenticator%20app.&text=This%20report%20also%20includes%20federated,are%20federated%20to%20Azure%20AD.){target="_blank"}找到。
