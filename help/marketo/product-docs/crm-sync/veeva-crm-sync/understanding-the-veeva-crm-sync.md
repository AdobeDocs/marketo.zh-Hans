...
通过几个简单的步骤，可以轻松地在Adobe Marketo Engage和[!DNL Veeva] CRM之间运行同步。
...
* 您在Marketo Engage中为 [!DNL Veeva]](/help/marketo/product-docs/crm-sync/salesforce-sync/setup/enterprise-unlimited-edition/step-2-of-3-create-a-salesforce-user-for-marketo-enterprise-unlimited.md){target="_blank"}输入的[凭据用于同步数据。 只有这些凭据有权限访问的数据才会得到同步。
