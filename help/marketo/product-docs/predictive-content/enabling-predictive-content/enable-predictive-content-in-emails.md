...
将电子邮件中的一个或多个图像设置为预测性图像，为每个收件人定制体验。
...
1. 您的图像现在具有预测性。 对其他图像（如果需要）重复步骤3和4。
...
1. 预览完电子邮件后，单击 **预览操作** 下拉菜单并选择 **批准并关闭**. 或者，如果您仍然需要编辑，请单击 **编辑草稿** 在右边。
...
如果您未使用 [电子邮件2.0](/help/marketo/product-docs/email-marketing/general/email-editor-2/email-editor-v2-0-overview.md){target="_blank"} 模板，只需将模板中的图像标记为Marketo可编辑的图像元素，即可向电子邮件添加预测内容。
...
以下是代码应呈现的外观示例（这只是一个示例，请不要完全复制下面的代码）。
