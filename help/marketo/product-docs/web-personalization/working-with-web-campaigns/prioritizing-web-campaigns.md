---
unique-page-id: 8782266
description: 了解如何在Marketo Engage中优先处理Web活动，包括优先处理Web活动。 使用本指南完成您的下一步。
title: 为 Web 营销活动设定优先级
exl-id: 18c43ba2-6d4a-4344-93be-3e1435742504
feature: Web Personalization
TQID: https://experienceleague.adobe.com/MwSVFrnnG-tBShEwxhapBPm0evix-J0ZbrgvWwBZSU8
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2:
  - id: ed6be6bb-75bb-4ea9-9a42-3bcaa65e1bcc
topic_v2:
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 177
ht-degree: 4%

---

# 为 Web 营销活动设定优先级 {#prioritizing-web-campaigns}

当两个或更多Web营销活动重叠时，通过设置优先级得分来优先处理Web营销活动。

>[!NOTE]
>
>**重叠的营销活动**
>
>在以下情况下，会发生Web营销活动重叠：
>
>* 两个或多个构件和/或对话框营销活动在同一页面上同时做出反应
>* 具有相同区域ID的两个或两个以上的In Zones在同一网页上同时反应
>
>In Zone营销活动和（小组件或对话框）营销活动可以在同一页面上做出反应。

1. 前往 **[!UICONTROL Web Campaigns]**。

   ![](assets/web-campaigns-hand-6.jpg)

   >[!NOTE]
   >
   >若要更轻松地查找所需的Web营销活动，请使用[筛选功能](/help/marketo/product-docs/web-personalization/working-with-web-campaigns/filter-web-campaigns.md)。

1. 在“编辑营销活动”页面中，设置[!UICONTROL Priority Score] （9999 =最高优先级1 =最低优先级）。

   ![](assets/image2015-7-9-20-3a20-3a58.png)

   >[!TIP]
   >
   >我们建议您仅在可能存在营销活动重叠时才使用营销活动[!UICONTROL Priority Score]，其中一个营销活动具有较高重要性。 您无需为每个营销活动设置优先级。

1. 保存或启动营销活动。

1. 查看[!UICONTROL Web Campaigns]页面中显示的[!UICONTROL Priority Score]。

![](assets/web-campaign-priority-score.jpg)
