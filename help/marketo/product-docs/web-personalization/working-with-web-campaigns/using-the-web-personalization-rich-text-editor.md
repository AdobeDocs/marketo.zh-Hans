---
unique-page-id: 4719179
description: 了解如何使用dnl web个性化在Marketo Engage中使用Web个性化富文本编辑器。 放心地迈出下一步。
title: 使用 Web 个性化富文本编辑器
exl-id: a07d8f2b-0ec6-4799-a430-0a5f7cf5f583
feature: Web Personalization
TQID: https://experienceleague.adobe.com/XmnR-DkqT-OfgqQndug3Yvos4EtYsv0U-Zmuq8rruQ8
product_v2: id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2: id: c5f60233-d5ea-4453-a799-0ad258b4d399id: d65b4a73-87a3-4d56-b638-74e74d9939ceid: ed6be6bb-75bb-4ea9-9a42-3bcaa65e1bcc
topic_v2: id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 185
ht-degree: 3%

---

# 使用[!DNL Web Personalization]富文本编辑器 {#using-the-web-personalization-rich-text-editor}

![](assets/one.png)

在[!UICONTROL Set Campaigns]页面中找到的[!DNL Web Personalization]富文本编辑器允许文本格式、链接和图像插入。 它包含与标准文字处理器类似的格式化选项。

富文本编辑器中的重要图标包括：

<table>
 <tbody>
  <tr>
   <td colspan="1"><img alt="--" src="assets/three.png" data-linked-resource-id="11386613" data-linked-resource-type="attachment" data-base-url="https://docs.marketo.com" data-linked-resource-container-id="4719179" title="--"></td>
   <td colspan="1"><strong>编辑HTML Source</strong></td>
   <td colspan="1">允许您查看HTML源代码</td>
  </tr>
  <tr>
   <td colspan="1"><img alt="--" src="assets/two.png" data-linked-resource-id="11386612" data-linked-resource-type="attachment" data-base-url="https://docs.marketo.com" data-linked-resource-container-id="4719179" title="--"></td>
   <td colspan="1"><p><strong>插入/编辑图像</strong><br></p></td>
   <td colspan="1">为编辑器中显示的图形插入图像URL</td>
  </tr>
  <tr>
   <td colspan="1"><img alt="--" src="assets/ds.png" data-linked-resource-id="12983884" data-linked-resource-type="attachment" data-base-url="https://docs.marketo.com" data-linked-resource-container-id="4719179" title="--"></td>
   <td colspan="1"><strong>从Design Studio插入图像</strong></td>
   <td colspan="1"><em>在</em>单击<strong>插入/编辑图像</strong>图标后，使用此图标从Marketo Design Studio中选择图像</td>
  </tr>
  <tr>
   <td colspan="1"><img alt="--" src="assets/four.png" data-linked-resource-id="11386614" data-linked-resource-type="attachment" data-base-url="https://docs.marketo.com" data-linked-resource-container-id="4719179" title="--"></td>
   <td colspan="1"><p><strong>插入/编辑链接</strong><br></p></td>
   <td colspan="1">用于向文本或图像添加超链接</td>
  </tr>
  <tr>
   <td colspan="1"><img alt="--" src="assets/tokens.png" data-linked-resource-id="12983883" data-linked-resource-type="attachment" data-base-url="https://docs.marketo.com" data-linked-resource-container-id="4719179" title="--"></td>
   <td colspan="1"><strong>插入/编辑令牌</strong></td>
   <td colspan="1">使用公司或人员令牌个性化您的Web营销活动</td>
  </tr>
 </tbody>
</table>

>[!NOTE]
>
>Web令牌使用[!DNL Marketo Database]中的[人员数据](/help/marketo/product-docs/web-personalization/using-web-segments/manage-person-data.md)，该数据在[!DNL Web Personalization]中的[!UICONTROL Account Settings] > [!UICONTROL Database]下管理。 如果Web令牌是[!UICONTROL Account Settings] > [!UICONTROL Database]中尚未包含的新数据库字段，则将自动添加该令牌，并且最多可能需要24小时才能激活。

>[!MORELIKETHIS]
>
>[使用富文本编辑器](/help/marketo/product-docs/email-marketing/general/understanding-the-email-editor/using-the-rich-text-editor.md)
