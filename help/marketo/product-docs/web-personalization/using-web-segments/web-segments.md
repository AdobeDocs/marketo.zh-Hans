...
在Web个性化中，访客可以匹配多个区段。 例如，如果存在针对美国访客的细分和针对金融公司的细分，则来自美国银行的Web访客将与&#x200B;**针对美国访客的细分和针对金融公司的细分相匹配。**

**图表：**  “区段”页根据区段（y轴）和区段名称（x轴）的访客数显示选定区段的条形图。
...
请参阅[如何创建和视图段标签](label-your-segment.md)
...
您创建的区段符合您在&#x200B;**设置区段**&#x200B;页面中定义的任何特定条件。 您还可以根据一系列标准来自定义您的细分，定位活动中的特定受众。
...
* **选择州或省以添加** -从下拉框中，选择要包括的美国州或加拿大省。您可以进行多个选择。
...
* 从以下参数中进行选择：
   ...
   * **包括** -选择您希望区段包括还是排除行业。
   * **选择要添加的行业** -选择要包含在该细分中的行业。该行业显示在下拉框下方。 您可以选择多个行业。
...
* **来自这些公司（特定名称）**
   ...
   * 您可以键入要目标的确切组织名称。 *建议*always *使用指定帐户列表，而不是手动键入名称以获得更好的匹配项（请参阅下文）。*

**指定帐户列表**

从[命名帐户列表](../../../product-docs/web-personalization/account-based-web-marketing/create-a-new-account-list.md)中进行选择，以细分关键目标帐户
...
选择数据库，然后从下拉列表中选择一个人员数据字段。 选择   **+**&#x200B;可添加下拉列表中的字段。
