...
在Web个性化中，访客可以匹配多个区段。 例如，如果存在适用于美国访客的区段和适用于金融公司的区段，则来自美国银行的Web访客将匹配 **两者** 美国访客分部及金融公司分部。

**图表：**  “区段”页面根据区段（y轴）的访客数和区段名称（x轴）显示选定区段的条形图。
...
您创建的区段符合您在 **设置区段** 页面。 您也可以根据条件的组合自定义区段，以定位营销活动中的特定受众。
...
>[!TIP]
>
>**我应选择哪些区段条件？ “AND”或“OR”？** OR在每个字段中用作附加选项。 潜在客户只需满足在每个字段内选择的多个标准中的一个标准，即可获得该区段的资格。 (例如，潜在客户可以来自美国。 *或* 来自国防工业的)。 AND函数作为此区段必须满足的附加必需参数。 （例如，潜在客户必须既来自美国，也来自国防工业）。 在每个分段用户档案中，每个单独的字段都可用作两个字段，即“AND”或“OR”，具体取决于所选的区段条件。

**行业** 在 **用户档案分段** 部分，选中旁边的框 **行业**.
...
* **来自这些公司（特定名称）**
   ...
   * 您可以键入要定位的确切组织名称。 *它是 _始终_ 建议使用指定帐户列表，而不是手动键入名称以获取更好的匹配项（请参阅下文）。

**指定帐户列表**

从中选择 [指定帐户列表](/help/marketo/product-docs/web-personalization/account-based-web-marketing/create-a-new-account-list.md) 以细分关键目标客户。
...
选择数据库，然后从下拉列表中选择人员数据字段。 选择 **+** 以从下拉列表中添加字段。
...
**Marketo电子邮件营销活动** 通过访客点击Marketo电子邮件并到达网站后的电子邮件反向链接，对营销活动进行分段和个性化。 按Marketo项目名称或营销活动名称进行分段，并继续通过电子邮件到Web的对话。 选择+以从下拉列表中添加字段。
...
* 已知 — 从已知访客的下拉框中选择此选项。 当访客在您的网站上提交表单并出现在Web个性化的“人员”页面中时，就知道该访客。
* 匿名 — 从匿名访客的下拉框中选择此选项。
...
* 特定访问次数 — 从下拉列表中选择此选项，以指定特定访客。
   ...
   * 在右侧的文本框中，输入要跟踪的访客编号。 单击访客（在访客页面中）和右侧面板上的Set Campaign时，可以找到唯一的Web Personalization访客标识号。 访客ID位于高级设置部分。 访客ID也可以在URL中找到（例如VISITOR=JZZJIFJNUI60PZ8Y97BHTY9BL8PKWS）。
...
* 选择要添加的反向链接 — 从下拉列表中，选择要跟踪的反向链接网站或添加您自己的反向链接。 选中后，反向链接将显示在下面的框中。 (使用 &#42; （表示允许使用通配符）

**包含页面**  — 跟踪潜在用户在您的网站上访问的特定页面。

* URL匹配 — 添加要跟踪的特定网页的URL。 您可以通过用分号分隔多个URL来添加它们。 (使用 &#42; （表示允许使用通配符）。

**排除页面**  — 排除您不希望在此区段中匹配的特定页面。 (使用 &#42; （表示允许使用通配符）。
...
* **访客类型**<br />
  **移动操作系统**  — 从下拉框中，从列出的一个或多个移动操作系统中选择。 选定的移动设备操作系统显示在下方。
...
* 浏览器版本 — 输入要添加到区段的浏览器版本。 您可以通过用逗号分隔多个版本来选择多个版本。 (使用 &#42; （表示允许使用通配符）。
...
>[!TIP]
>
>**使用通配符 —** 何时希望包含任何搜索词或URL，其中包含“[google.com](https://google.com)”或“搜索词产品”，我们将其称为通配符，应当用星号输入 — 这个小家伙&#42;  — 两端。 所以任何来自 [google.com](https://google.com) 应输入为 &#42; [google.com](https://google.com)&#42;
