...
Add the landing page CNAME they sent you to your DNS record, so that `[YourLandingPageCNAME]` points to the unique Account String that is assigned to your Marketo landing pages. 登录到您的域注册机构的网站，然后输入登陆页面CNAME和帐户字符串。 Typically, this involves three fields:
...
When your Marketing group uses Marketo to send test emails (a best practice before sending out email blasts), the test emails are sometimes blocked by anti-spam systems that rely on sender IP addresses to verify that the email is valid. 要确保这些测试电子邮件送达，请将Marketo添加到您的允许列表。
...
MX记录允许您接收来自的域的邮件，以处理回复和自动应答器。 如果您从公司域发送，则可能已经配置了此设置。 If not, you can usually set it up to map to your corporate domain’s MX record.
