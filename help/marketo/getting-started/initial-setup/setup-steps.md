...
`5)` 我们需要确保用于发送电子邮件的 FROM 域名 **`[FromDomain1]`**、**`[FromDomain2]`** 等均存在有效的 MX 记录。请协助确认。如果不存在，请配置其映射至公司主域名的 MX 记录。这样可以确保我们能够正常处理 Marketo 电子邮件的回复和自动回复。
...
这可能是整个设置过程中最令人期待的部分。现在是时候将您在 CRM 中存储的所有潜在客户和联系人导入 Marketo 了！
